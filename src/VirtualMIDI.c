...
static int wFiles(FILE* sheetStream, FILE* logStream, uint8_t* buffer, long fileLength) {
    ...
    struct noteRec* allNoteRecs = (void*)0;
    size_t noteRecsCount = 0;
    size_t noteRecsCapacity = 0;
    ...
    /***********************************************************/
    if (logStream)
        fprintf(logStream, "File length is %li bytes", fileLength);
    ...
    for (int i = 0; i < fileLength; i++)
    {
        ...
        if (i + 4 < fileLength)
            if (buffer[i] == 0x4D && buffer[i + 1] == 0x54) // "MT"
            {
                ...
                {
                    if (parseMThd(logStream, buffer, i + 4) != 0)
                        return 1;
                    if (logStream)
                        fprintf(logStream, "\n");
                }
                ...
                {
                    if (parseMTrk(logStream, buffer, i + 4,
                        &allNoteRecs, &noteRecsCount, &noteRecsCapacity,
                        &allTsRecs, &tsRecsCount, &tsRecsCapacity) != 0)
                    {
                        free(allNoteRecs); // nvm, cuz it encounters an error
                        allNoteRecs = (void*)0;
                        free(allTsRecs);
                        allTsRecs = (void*)0;
                        return 1;
    ...
    /***********************************************************/
    if (noteRecsCount > 1)
        qsort(allNoteRecs, noteRecsCount, sizeof(struct noteRec), compNoteRec);
    if (tsRecsCount > 1)
        qsort(allTsRecs, tsRecsCount, sizeof(struct TsRec), compTsRec);

    // Auto-transpose into C2-C7 to avoid invalid keys.
    const int32_t notePlayableMin = 48; // C2
    const int32_t notePlayableMax = 84; // C7
    int32_t minKeyValue = 127;
    int32_t maxKeyValue = 0;
    _Bool hasClosedNotes = 0;
    for (size_t i = 0; i < noteRecsCount; i++)
    {
        if (!allNoteRecs[i].closed)
            continue;
        hasClosedNotes = 1;
        if (allNoteRecs[i].keyValue < minKeyValue)
            minKeyValue = allNoteRecs[i].keyValue;
        if (allNoteRecs[i].keyValue > maxKeyValue)
            maxKeyValue = allNoteRecs[i].keyValue;
    }
    ...
    if (hasClosedNotes)
    {
        int32_t minShift = notePlayableMin - minKeyValue;
        int32_t maxShift = notePlayableMax - maxKeyValue;
        ...
        {
            // if song range is wider than note range pick center
            int32_t sourceMid = (minKeyValue + maxKeyValue) / 2;
            int32_t targetMid = (notePlayableMin + notePlayableMax) / 2;
    ...
    /***********************************/
    size_t tsIndex = 0;
    for (size_t i = 0; i < noteRecsCount;)
    {
        if (!allNoteRecs[i].closed)
        {
            i++;
        ...
        if (tsRecsCount > 0)
        {
            while (tsIndex + 1 < tsRecsCount && allTsRecs[tsIndex + 1].absTick <= allNoteRecs[i].startTick)
                tsIndex++;

            if (allTsRecs[tsIndex].absTick <= allNoteRecs[i].startTick)
            {
        ...
        size_t clusterEnd = i;
        while (clusterEnd + 1 < noteRecsCount)
        {
            size_t cand = clusterEnd + 1;
            while (cand < noteRecsCount && !allNoteRecs[cand].closed)
                cand++;
            if (cand >= noteRecsCount)
                break;

            int32_t deltaCluster = allNoteRecs[cand].startTick - allNoteRecs[clusterEnd].startTick;
        ...
        if (isCluster)
        {
            ...
            printf("[");
            if (sheetStream) fprintf(sheetStream, "[");
        }

        for (size_t k = i; k <= clusterEnd; k++)
        {
            int32_t transposedKeyValue = (int32_t)allNoteRecs[k].keyValue + autoTranspose;
            // to keep a valid note key when exceedss range.
            while (transposedKeyValue < notePlayableMin)
                transposedKeyValue += 12;
            while (transposedKeyValue > notePlayableMax)
                transposedKeyValue -= 12;
            ...
            if (trOctave <= 8 && trKeyNum <= 11 && keyboard[trOctave][trKeyNum][0] != '\0')
                noteKey = keyboard[trOctave][trKeyNum];

            unsigned char keyChar = (unsigned char)noteKey[0];
            if (isCluster)
            {
                if (keyChar == '\0')
                    continue;
            ...
            if (sheetStream) fprintf(sheetStream, "%s", noteKey);
            printf("%s", noteKey);
        }
        ...
        while (nextI < noteRecsCount && !allNoteRecs[nextI].closed)
            nextI++;

        int32_t closeDelta = INT32_MAX;
        if (nextI < noteRecsCount)
            closeDelta = allNoteRecs[nextI].startTick - allNoteRecs[clusterEnd].startTick;
...
/*                            PARSE MIDI HEADER                              */
/*****************************************************************************/
static _Bool parseMThd(FILE* logStream, uint8_t* buffer, int startOffset) {
    header.length = 0;
    header.format = 0;
    header.nTrks = 0;
    ...
    */
    ...
    // check the kind of format and delta-time
    ...
    {
        header.divFormat = 1; // <-- no different for now
        //header.FPS = (int8_t)header.division >> 8;
        //header.TPF = header.division & 0xFF;
    }
    else // 2 or anything else that doesnt work for now
    {
        if (logStream)
            fprintf(logStream, "\nunsupported MIDI file format: %i\n", divisionBits[15]);
        return 1;
    }
...
/*                             PARSE MIDI TRACK                              */
/*****************************************************************************/
#define trackLength track.length
#define eventDeltaTime track.event.deltaTime
#define eventDeltaTimeMS track.event.deltaTimeMS
#define eventAbsTick track.event.absTick
#define eventType track.event.type
#define eventSubtype track.event.subtype
#define eventLength track.event.length

static _Bool parseMTrk(
    FILE* logStream, uint8_t* buffer, int lengthStart,
    struct noteRec** allNoteRecs, size_t* noteRecsCount, size_t* noteRecsCapacity,
    ...
    // if eventLength occupies the bytes 20 and 25 then afterEventLength is 26 (cuz of how I did the VLQ function)
    ...
    for (int i = 3; i >= 0; i--)
    {
        trackLength |= buffer[lengthStart + 3 - i] << (8 * i); // managed to use bitwise logic instead from down here :DD
    }

    int dataStart = lengthStart + 4;
    int trackEnd = dataStart + (int)trackLength;
    int i = dataStart;
    uint8_t runningStatus = 0;
    int32_t noteRecIndex[16][128];

    struct noteRec* noteRecs = (void*)0;
    struct noteRec noteRec;
    struct TsRec tsRec;

    size_t noteRecCount = 0;
    size_t noteRecCapacity = 0;
    ...
    */
    if (logStream)
        fprintf(logStream, "║ MTrk start(@B%i) | MTrk data length(from B%i): %iB\n", lengthStart - 4, dataStart, trackLength);

    /************************************ HANDLE EVENTS **************************************/
    while (i < trackEnd)
    {
        calcVLQ(buffer, i);
        eventDeltaTime = VLQ.VLQresult;
        eventDeltaTimeMS = (eventDeltaTime * track.microsPT) / 1000;
        eventAbsTick += eventDeltaTime;
        ...
        _Bool isChannelEvent = ((status & 0xF0) >= 0x80 && (status & 0xF0) <= 0xE0); // Channel events
        _Bool isMetaEvent = (status == 0xFF); // Meta: FF tt len data
        _Bool isSysexEvent = (status == 0xF0 || status == 0xF7); // SysEx: F0/F7 len data

        /*****************************************************************************/
        /*                              META    EVENTS                               */
        /*****************************************************************************/
        if (isMetaEvent) // Meta: FF tt len data
        {
            if (cursor >= trackEnd)
            {
                if (logStream)
            ...
            eventSubtype = buffer[cursor++];
            calcVLQ(buffer, cursor);
            eventLength = VLQ.VLQresult;
            int afterEventLength = VLQ.VLQi;
            //int eventLenBytes = afterEventLength - cursor;
            int dataEnd = afterEventLength + (int)eventLength;
            if (dataEnd > trackEnd)
            {
                if (logStream)
            ...
            {
                fprintf(logStream, "\n║ META");
                fprintf(logStream, " | deltaTime: 0x%06X(%uticks)(%ims)(@B%i-B%i)", eventDeltaTime, eventDeltaTime, eventDeltaTimeMS, i, afterDeltaTime - 1);
                fprintf(logStream, " | absTick: 0x%06X(%iticks)", eventAbsTick, eventAbsTick);
                fprintf(logStream, " | type: 0x%02X(@B%i) | subtype: 0x%02X(@B%i)", eventType, afterDeltaTime, eventSubtype, cursor - 1);
            }

            /***********************************/
            /*           SET TEMPO             */
            /***********************************/
            if (eventSubtype == 0x51) // "Set Tempo" Meta Event
            {
                if (logStream)
                {
                    fprintf(logStream, " | SET_TEMPO");
                    fprintf(logStream, " | eventLen: 0x%06X(%i)(@B%i-B%i)", eventLength, eventLength, cursor, afterEventLength - 1);
                }
                track.microsPQN = 0;
                track.microsPT = 0;
                if (header.divFormat == 0)
                {
                    ...
                    {
                        track.microsPQN |= buffer[(afterEventLength + eventLength - 1) - j] << (8 * j);
                    }
            ...
            /*         TIME SIGNATURE          */
            /***********************************/
            else if (eventSubtype == 0x58) // "Time Signature" Meta Event
            {
            ...
            /*          KEY SIGNATURE          */
            /***********************************/
            else if (eventSubtype == 0x59) // "Key Signature" Meta Event
            {
            ...
            /*          TEXT  EVENT            */
            /***********************************/
            else if (eventSubtype == 0x01) // "Text Event" Meta Event
            {
                char* text = malloc(eventLength + 1);
                for (int j = 0; j < eventLength; j++)
            ...
            /*        COPYRIGHT NOTICE         */
            /***********************************/
            else if (eventSubtype == 0x02) // "Copyright Notice" Meta Event
            {
                char* text = malloc(eventLength + 1);
                for (int j = 0; j < eventLength; j++)
                    text[j] = buffer[afterEventLength + j];
            ...
            /*          SEQUENCE NAME          */
            /***********************************/
            else if (eventSubtype == 0x03) // "Sequence/Track Name" Meta Event
            {
                char* text = malloc(eventLength + 1);
                for (int j = 0; j < eventLength; j++)
                    text[j] = buffer[afterEventLength + j];
            ...
            /*         END OF TRACK            */
            /***********************************/
            else if (eventSubtype == 0x2F) // "End Of Track" Meta Event
            {
                if (logStream)
        ...
        /*                             CHANNEL  EVENTS                               */
        /*****************************************************************************/
        if (isChannelEvent) // Channel events
        {
            ...
            _Bool isNoteOn = (status >= 0x90 && status <= 0x9F);  // 0x9# Note ON
            _Bool isNoteOff = (status >= 0x80 && status <= 0x8F); // 0x8# Note OFF
            _Bool hasVel = (buffer[dataStartByte + 1]);
            ...
            eventLength = (uint32_t)channelDataBytes;
            eventSubtype = buffer[dataStartByte];
            ...
            if (isNoteOn && hasVel)
            {
                ...
                track.notes[channel][keyValue] = eventAbsTick;
                noteRec.channel = channel + 1;
                noteRec.keyValue = keyValue;
                noteRec.keyNum = keyNum;
                noteRec.octave = octave;
                noteRec.startTick = eventAbsTick;
                noteRec.endTick = -1;
                noteRec.durationTick = -1;
                noteRec.durationMS = -1;

                /*
                printf("noteRec | channel: %u | keyValue: %u | keyNum: %u | octave: %u | startTick: %li | endTick: %li | durationTick: %li | durationMS: %li | timeSignature: %i/2^%i\n",
                    noteRec.channel, noteRec.keyValue, noteRec.keyNum, noteRec.octave, noteRec.startTick, noteRec.endTick, noteRec.durationTick, noteRec.durationMS, timeSigN, timeSigD);
                */

                noteRec.closed = 0;
                if (appendRec((void**)&noteRecs, &noteRecCount, &noteRecCapacity, sizeof(*noteRecs), &noteRec))
                {
                    if (logStream)
                        fprintf(logStream, "\n | failed to allocate noteRec buffer");
                    break;
                }
                noteRecIndex[channel][keyValue] = (int32_t)(noteRecCount - 1);
                ...
                if (logStream)
                {
                    ...
                    fprintf(logStream, " | eventLen: %i(@B%i-B%i)", eventLength, dataStartByte, dataEnd - 1);
            ...
            else if (isNoteOff || (isNoteOn && !hasVel))
            {
                ...
                if (track.notes[channel][keyValue] != -1)
                {
                    ...
                    if (recIndex >= 0 && (size_t)recIndex < noteRecCount)
                    {
                        noteRecs[recIndex].endTick = endTick;
                        noteRecs[recIndex].durationTick = noteDuration;
                        noteRecs[recIndex].durationMS = noteDurationMS;
                        noteRecs[recIndex].closed = 1;
                    }
        ...
        /*                              SYSEX   EVENTS                               */
        /*****************************************************************************/
        if (isSysexEvent) // SysEx: F0/F7 len data
        {
            calcVLQ(buffer, cursor);
            eventLength = VLQ.VLQresult;
            int afterEventLength = VLQ.VLQi;
            ...
            if (logStream)
            {
                ...
                fprintf(logStream, " | deltaTime: 0x%06X(%uticks)(%ims)(@B%i-B%i)", eventDeltaTime, eventDeltaTime, eventDeltaTimeMS, i, afterDeltaTime - 1);
                fprintf(logStream, " | absTick: 0x%06X(%iticks)", eventAbsTick, eventAbsTick);
                fprintf(logStream, " | type: 0x%02X(@B%i)", eventType, afterDeltaTime);
                fprintf(logStream, " | eventLen: 0x%06X(%i)(@B%i-B%i)", eventLength, eventLength, cursor, afterEventLength - 1);
            }
            i = dataEnd;
            continue;
    ...
    /**************************** MERGE THIS TRACK INTO GLOBAL NOTE BUFFER ****************************/
    for (size_t recI = 0; recI < noteRecCount; recI++)
    {
        if (!noteRecs[recI].closed)
            continue;
        if (appendRec((void**)allNoteRecs, noteRecsCount, noteRecsCapacity, sizeof(**allNoteRecs), &noteRecs[recI]))
        {
            free(noteRecs);
            noteRecs = (void*)0;
            if (logStream)
                fprintf(logStream, "\n | failed to allocate merged note buffer");
            return 1;
        }
    }
    free(noteRecs);
...
static int compNoteRec(const void* a, const void* b) {
    const struct noteRec* ra = (const struct noteRec*)a;
    const struct noteRec* rb = (const struct noteRec*)b;
...
static void numToKey(uint8_t number, uint8_t* resultName, uint8_t* resultOctave) {
    uint8_t octave = 0;  // 0 to 8
    uint8_t minInOct = 0; // 24 (0)
    uint8_t maxInOct = 0; // 35 (11)
    uint8_t key = 0;
